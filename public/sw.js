 ...
 *
 * Two kinds arrive here: a message, and a ring. They are told apart by
 * `payload.kind === 'call'` and differ in how long they stay on screen and
 * which tag they occupy — see showUnlessAlreadyLooking. Tapping either one
 * opens the conversation; for a ring, the app then asks the caller to repeat
 * the offer it missed (`ring-request` in lib/calls/controller.js), which is
 * what makes an answered notification connect to a real call.
 *
...
async function showUnlessAlreadyLooking(title, payload, conversationId) {
  ...
  return self.registration.showNotification(title, {
    ...
    //
    // A RING GETS ITS OWN TAG so it is not silently replaced by a message that
    // arrives while the phone is ringing — losing the one notification that is
    // only useful for the next forty seconds.
    tag: isCall
      ? (conversationId ? `call:${conversationId}` : 'call')
      : (conversationId ? `conv:${conversationId}` : 'heychat'),
    renotify: true,
    // A missed message can wait; a missed call cannot. This keeps the ring on
    // screen until it is acted on, rather than letting it auto-dismiss while
    // the caller is still waiting.
    requireInteraction: isCall,
    // Ignored where the platform does not support it. The long-short-long
    // pattern reads as a ring rather than as a message buzz.
    vibrate: isCall ? [400, 150, 400, 150, 400] : undefined,
    // A real PNG, not the inline SVG this used to point at: support for SVG
    // notification icons is uneven, and where it is missing the platform
    // silently substitutes its own default. Generated by scripts/build-icons.mjs.
    icon: '/icon-192.png',
